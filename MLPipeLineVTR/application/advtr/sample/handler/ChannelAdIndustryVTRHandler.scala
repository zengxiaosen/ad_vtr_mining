...
class ChannelAdIndustryVTRHandler(val featureHandlerName: String = "ChannelAdIndustryVTRHandler") extends FeatureHandler{
  private val ChannelAdIndustryVTRFeatureConf = VTRSampleConfig.parentPath + "featureHandlerConf/cpv_channel_adIndustryVTR.conf"
  private var maxFeatureIndex: Int = 0
  private val numOfFields:Int = 2
  private var unSeenFeature: Feature = _
  private var ChannelAdIndustryVTRMap: Map[String,Double] = new HashMap[String,Double]()
  ...
  override def initFeatureHandler(initFeatureIndex: Int): Int = {
    ...
    try{
      source = Source.fromFile(ChannelAdIndustryVTRFeatureConf)
      val lineItr = source.getLines()
      for(line <- lineItr){
        val contents = line.split("\\t")
        if(contents.length == numOfFields){
          if(!ChannelAdIndustryVTRMap.contains(contents(0).trim)){
            //val localIndex = 1
            ChannelAdIndustryVTRMap.+=(contents(0).trim -> contents(1).trim.toDouble)
          }
        }
      }
    }catch{
      ...
      unSeenFeature = Feature(initFeatureInd + 1, "ChannelAdIndustryVTR", if(discreteLen <= 1)0.0 else 1.0)

      var index = 1
      var featureIndex = initFeatureInd + 1
      while(featureIndex <= maxFeatureIndex){
        System.out.println(s"feature:${featureIndex},[ChannelAdIndustryVTR]_[$index]")
        featureIndex += 1
        index += 1
      }
      if(source != null) {
  ...
  override def extractFeature(dataInstance: DataInstance): ArrayBuffer[Feature] = {
    ...
    if(dataInstance.isInstanceOf[VTRDataInstance]){
      ...
      val adIndustry = vtrDataInstance.adIndustry
      if(Channel != null && adIndustry != null) {
        val ChannelAdIndustry = Channel + "_" + adIndustry
        if (ChannelAdIndustryVTRMap.contains(ChannelAdIndustry)) {
          val vtr = ChannelAdIndustryVTRMap(ChannelAdIndustry)
          ...
          if(discreteLen > 1) {
            ...
            features.+=(Feature(initFeatureInd + discreteIndex + 1, "ChannelAdIndustryVTR", 1.0))
          }else{
            features.+=(Feature(initFeatureInd + 1, "ChannelAdIndustryVTR", vtr))
          }
        } else {
